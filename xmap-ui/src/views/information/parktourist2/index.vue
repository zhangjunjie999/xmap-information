


<template >
  <div class="box" ref="myImg">

    <!-- 顶部 -->
    <div class="body-top">
      <div class="station-name">站点名称： {{ stationName }}</div>
      <!--这个需求是有消息时才会让这个功能显示，所以使用了v-if-->
      <div class="bs-sysMsg" v-if="systemMsg.length > 0">
        <i class="el-alert__icon el-icon-warning"></i>
        <div class="msg__content">
          <el-carousel height="20px" direction="horizontal" indicator-position="none" :autoplay="true">
            <el-carousel-item v-for="item in systemMsg" :key="item.id">
              <a href="javascript:void(0)" class="item">{{ item.title }}</a>
            </el-carousel-item>
          </el-carousel>
        </div><br>
      </div>
    </div>

    <!-- 主体 -->
    <div class="body-main">
      <div class="weather-info">
        <div class="time"><span> {{ newTime }} </span></div>
        <div class="weather"><span> {{ weather }} {{ temperature }} </span></div>

      </div>

      <div class="park-info">
        <!-- part1 -->
        <!-- <div class="p1-p2">

               </div> -->
        <div class="park-info-p1">
          <span class="park-left">
            <div class="park-icon">
              <img class="img" src="./image/rightturn.png" />
            </div>
          </span>
          <span class="park-right">

            <div class="park-distance">
              {{ turn }}{{ distance }}米
            </div>
            <div class="park-station">
              {{ stationName }}
            </div>


          </span>
        </div>
        <!-- part2 -->
        <div class="park-info-p2">
          <div class="restNumber"> 剩余车位 ：
            <span class="restNumberColor">
              {{ restNumber }}
            </span>
            个
          </div>
          <div class="usedNumber"> 已用车位 ：
            <span class="usedNumberColor">
              {{ usedNumber }}
            </span>
            个
          </div>
          <!-- <span>建议停车场</span> <span>空余车位</span> -->
        </div>
        <!-- part3 -->
        <div class="park-info-p3">
          <el-table :data="parkList"
            :cell-style="{ fontWeight: 'bold', fontSize: '100%', background: 'white', borderBottom: '1px solid white' }"
            :header-cell-style="{ color: 'black', fontWeight: 'bold', fontSize: '100%', background: 'white', borderBottom: '3px solid #5B9BD8', borderColor: '#5B9BD8', border: '3px solid #5B9BD8 none none none' }"
            style="border-Bottom:1px solid white">

            <el-table-column label="建议停车场" align="center" prop="parkName" style="border-bottom:1px solid red">
              <template slot-scope="scope">
                <span style="color: black ;font-Weight: bold ;">{{ scope.row.name }}</span>
              </template>
            </el-table-column>


            <el-table-column label="空余车位" align="center" prop="emptyNumber">
              <template slot-scope="scope">
                <span style="color: red; font-Size:100%;">{{ scope.row.value }}</span>
              </template>
            </el-table-column>

          </el-table>
          <!-- <div class="first-line">
            <div class="first-line-left">建议停车场</div>
            <div class="first-line-right">空余车位</div>
          </div>
          <div class="second-line">
            <div class="second-line-left">杜鹃广场停车场</div>
            <div class="second-line-right"> <span style="color: red; font-Size:100%;">  {{ numberDujuan }}</span></div>
          </div>
          <div class="third-line">
            <div class="third-line-left">红歌广场停车场</div>
            <div class="third-line-right"><span style="color: red; font-Size:100%;"> {{ numberHongge }}</span></div>
          </div>
          <div class="fourth-line">
            <div class="fourth-line-left">会展中心停车场</div>
            <div class="fourth-line-right"><span style="color: red; font-Size:100%;"> {{ numberHuizhan }}</span></div>
          </div>
          <div class="fifth-line">
            <div class="fifth-line-left">游客服务中心3号停车场</div>
            <div class="fifth-line-right"><span style="color: red; font-Size:100%;"> {{ numberTourist3 }}</span></div>
          </div> -->
          <!-- <div class="sixth-line">
            <div class="sixth-line-left">北山革命烈士纪念碑停车场</div>
            <div class="sixth-line-right"><span style="color: red; font-Size:100%;">23</span></div>
          </div> -->
        </div>
      </div>
    </div>
    <!-- 底部 -->
    <div class="body-down">

      <div class="phone-info">
        <div class="phone-info-h">救援电话：{{ phone }}</div>
        <i class="el-icon-data-line" @click="toggleFullScreen"></i>
      </div>


    </div>

  </div>
</template> 
<script>
import { listParking,  weatherParking, getWeather } from "@/api/information/parking";

import axios from "axios";
import screenfull from "screenfull";

export default {
  name: 'Home',
  data() {
    return {
      // 滚屏消息
      // systemMsg: [
      //     {id:1,title:'公告：近10日 拜登做了10件大事'},
      //     {id:2,title:'通知：全民带货？小红书外链淘宝权限将大范围开放'},
      //     {id:3,title:'公告：贾跃亭FF将在纳斯达克上市 股票代码为FFIE'}
      // ], 
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
      // 非单个禁用
      single: true,
      // 非多个禁用
      multiple: true,
      // 显示搜索条件
      showSearch: true,
      // 总条数
      total: 0,
      // 停车场引导牌管理表格数据
      parkingList: [],
      systemMsg: [
        { id: 1, title: '公告：欢迎光临井冈山' },

      ],
      newTime: "",
      //天气
      weather: '晴 ',
      //温度
      temperature: '',
      //风向
      winddirection: '',
      //风力
      windpower: '',
      //湿度
      humidity: '',
      //报告时间
      reporttime: '',
      //电话
      phone: '',
      //方向
      turn: '右前方',
      //距离
      distance: '5',
      //站点名
      stationName: '游客服务中心2号停车场',
      //车位数
      restNumber: '36',
      usedNumber: '114',
      //时间
      dateTime: '',
      //星期
      weekDay: '',
      //杜鹃广场停车场剩余车位数
      numberDujuan: '',
      //红歌广场停车场剩余车位数
      numberHongge: '',
      //会展中心停车场剩余车位数
      numberHuizhan: '',
      //游客服务中心3号停车场剩余车位数
      numberTourist3: '',

      // 查询参数
      queryParams: {
        pageNum: 1,
        pageSize: 10,
        siteName: null,
        notice: null,
        weather: null,
        icon: null,
        distance: null,
        location: null,
        availablePark: null,
        occupiedPark: null,
        nearbyPark: null,
        availableSpace: null,
        emergencyPhone: null,
        dateTime: null,
        weekDay: null
      },
      parkList: [{ name: "杜鹃广场停车场", value: "13" },
      { name: "红歌广场停车场", value: "10" },
      { name: "会展中心停车场", value: "3" },
      { name: "农贸市场停车场", value: "6" },
      ],
      //天气数据
      weatherList: [],
      queryParams2: {
        pageNum: 1,
        pageSize: 10,
        weatherId: null,
        weatherInfo: null,
        weatherHigh: null,
        weatherLow: null,
        weatherTime: null,
      },

    }
  },
  create() {

    this.getList();
   
    this.getWeatherList();

  },
  mounted() {
    this.getList();
    this.getWeather();
    this.getWeatherList();
    this.flash();
  },


  // 在Vue实例销毁前，清除我们的定时器
  beforeDestroy() {

    if (this.times) {
      clearInterval(this.getTimesInterval);
    }

  },
  methods: {

    /** weatherList from database */
    getWeatherList() {
      getWeather(this.queryParams2).then(response => {
        this.loading = false;
        this.weatherList = response.rows;

        const item = this.weatherList.find(item1 => item1.weatherId === 1);
        this.weather = item.weatherInfo + " " + item.weatherLow + "~" + item.weatherHigh;
      });
    },
    /** weather */
    getWeather() {
      console.log("getWeather方法正在被调用")
      // console.log("parkingList 数据结果0： "+this.parkingList)
      this.loading = true;
      weatherParking().then(response => {
        this.loading = false;
        // this.weather=response[0].type+" "+response[0].low+"~"+response[0].high;  
        console.log("getWeather方法停止被调用")
      });
    },



    /**定时器 */
    flash() {
      const timer = setInterval(() => {
        // 获取后端数据    
        this.getList();
        this.getWeatherList();
      }, 1000 * 60 * 3);
      const timer2 = setInterval(() => {
        // 获取天气数据   
        this.getWeather();
      }, 1000 * 60 * 10 * 3);
      // 通过$once来监听定时器，在beforeDestroy钩子可以被清除。
      this.$once('hook:beforeDestroy', () => {
        clearInterval(timer);
        clearInterval(timer2);
      })
    },
    /** 查询停车场引导牌管理列表 */
    getList() {
      console.log("getList方法正在被调用")
      // console.log("parkingList 数据结果0： "+this.parkingList)
      this.loading = true;
      listParking(this.queryParams).then(response => {
        this.parkingList = response.rows;
        this.total = response.total;
        this.loading = false;
        // console.log("parkingList 数据结果： "+this.parkingList)
        const item = this.parkingList.find(item1 => item1.siteName === this.stationName)
        // console.log("item 数据结果： "+item)

        //天气
        // this.weather = item.weather;

        this.phone = item.emergencyPhone;
        this.distance = item.distance;
        this.restNumber = item.availablePark;
        this.usedNumber = item.occupiedPark;
        this.systemMsg[0].title = item.notice;
        this.dateTime = item.dateTime;
        // console.log("this.dateTime: "+ this.dateTime);
        this.weekDay = item.weekDay;
        // console.log("this.weekDay: "+ this.weekDay);
        this.newTime = this.weekDay;
        this.nearbyPark = item.nearbyPark;

        const str1 = item.nearbyPark;
        var arr1 = eval('(' + str1 + ')');

        this.parkList = arr1;
        console.log("getList方法停止被调用")
      });
    },
    makeFullScreen() {
      this.toggleFullScreen();
    },
    //页面全屏
    toggleFullScreen() {
      if (screenfull.isEnabled) {
        screenfull.toggle(this.$refs.myImg);
      }
    },


    //时间实现方法
    getNowTime() {
      // console.log("时间更新");
      // var date = new Date();
      console.log("时间更新:" + this.weekDay);
      // var date = this.dateTime;
      // ':' + date.getSeconds()
      // let wk = date.getDay()
      // let weeks = ['星期日', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']
      // let week = weeks[wk]
      // this.nowWeek = week
      // var time = date.getFullYear() + '年' + (this.addZero((date.getMonth() + 1))) + '月' + this.addZero(date.getDate()) + ' 日 ' + this.weekDay + ' ' + this.addZero(date.getHours()) + ':' + this.addZero(date.getMinutes());
      this.newTime = this.weekDay;
    },
    //根据自己的需求，看要不要在时间不大于10的时候在前面补0，如果需要直接this.addZero(date.getMinutes()),其它与之相同，如果不需要删掉addZero（）方法即可。
    //小于10的拼接上0字符串
    addZero(s) {
      return s < 10 ? ('0' + s) : s;
    },
  },

  // 修改 table cell边框的背景色
  tableCellStyle({ row, column, rowIndex, columnIndex }) {
    if (column.property === 'emptyNumber') {
      return {
        color: '#ec0b0b'
      }
    } else {
      return {
        color: '#FFFFFF'
      }
    }
  },
  // 修改 table header cell的背景色
  tableHeaderCellStyle() {
    return ' color: #606266; font-weight: bold; font-size:100%;'
  }

}
</script>
<style>
html,
body {
  width: 100%;
  height: 100%;
}

/*轮翻消息*/
.bs-sysMsg {
  position: relative;
  display: flex;
  width: 100%;
  padding: 8px 12px;
  margin-bottom: 10px;
  border-radius: 2px;
  color: hwb(357 28% 13%);
  background-color: #DFDFDF;
  overflow: hidden;
  opacity: 1;
  align-items: center;
  transition: opacity .2s;
}

.bs-sysMsg .msg__content {
  display: table-cell;
  padding: 0 8px;
  width: 100%;
  height: 50%;
}

.bs-sysMsg .msg__content a.item {
  color: #e01717;
  font-size: 17px;
  opacity: 0.75;
  font-weight: bold;
}

.bs-sysMsg .msg__content a.item:hover {
  text-decoration: underline;
}

.time {
  color: #f7f7f7;
  text-align: center;
}

.weather {
  color: #f7f7f7;
  text-align: center;
}

.box {
  margin: auto;
  text-align: center;
  height: 90%;
  width: 100%;
  background-color: #FAFAFA;
  position: fixed;
}

.body-top {
  background-color: #FAFAFA;
  height: 10%;
}

.station-name {
  text-align: center;
  height: 50%;

  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(10%);
}

.body-main {
  background-color: #5B9BD8;
  height: 85%;
  width: 100%;
  margin: auto;
}

.weather-info {
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(10%);
  height: 10%;
}

.park-info {

  height: 95%;
  width: 95%;
  margin: auto;
}

.park-info-p1 {
  border-style: solid;
  /* 上 右 下 左 */
  border-color: #5B9BD8 #5B9BD8 #FFFFFF #5B9BD8;
  background-color: #FFFFFF;
  height: 30%;
  width: 99%;
}

.park-info-p2 {
  border-style: solid;
  border-color: #FFFFFF #5B9BD8 #5B9BD8 #5B9BD8;
  background-color: #FFFFFF;
  height: 10%;
  width: 99%;
}

.park-info-p3 {

  height: 30%;
  width: 99%;
}

.park-left {
  height: 90%;
  width: 50%;
  text-align: center;
  float: left;
}

.park-icon {
  height: 90%;
  width: 60%;
  margin: auto;
  display: inline-block;
}

.park-right {
  height: 90%;
  width: 50%;

  margin: auto;
  float: left;
}

.park-distance {
  height: 45%;
  width: 100%;
  text-align: center;
  position: relative;
  top: 5%;
  /*偏移*/
  transform: translateY(50%);

  float: left;
  color: black;
  font-Size: 100%;
  font-weight: bold;



}

.park-station {
  height: 45%;
  width: 100%;
  text-align: center;
  float: left;
  color: black;
  font-Size: 100%;
  font-weight: bold;

}

.img {
  width: 90%;
  height: 90%;
  padding: 10px 0;
  text-align: center;
  float: left;

}


.body-down {
  background-color: #5B9BD8;
  height: 25%;
}

.phone-info {
  background-color: #5B9BD8;
  height: 50%;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(10%);
  font-size: calc(100vw * 30 / 1920);

}

.phone-info-h {
  background-color: #5B9BD8;
  color: #f6f3f3;

}

.restNumber {
  height: 50%;
  font-weight: bold;
  font-size: 100%;
}

.usedNumber {
  height: 50%;
  font-weight: bold;
  font-size: 100%;
}

.restNumberColor {
  color: #e01717;
}

.usedNumberColor {
  color: #17aa28;
}

.test {
  color: #ec0b0b;
}

.el-table::before {
  background-color: white;
}

.first-line {
  height: 30%;
  width: 100%;
  background-color: #FFFFFF;
  border-style: solid;
  border-color: #5B9BD8 #5B9BD8 #5B9BD8 #5B9BD8;

}

.first-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(40%);
}

.first-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(40%);
}

.second-line {
  height: 30%;
  width: 100%;
  border-style: solid;
  background-color: #FFFFFF;
  border-color: #5B9BD8 #5B9BD8 #FFFFFF #5B9BD8;

}

.second-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);

}

.second-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  font-size: 100%;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);
}

.third-line {
  height: 30%;
  width: 100%;
  border-style: solid;
  background-color: #FFFFFF;
  border-color: #FFFFFF #5B9BD8 #FFFFFF #5B9BD8;
}

.third-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);

}

.third-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  font-size: 100%;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);
}

.fourth-line {
  height: 30%;
  width: 100%;
  border-style: solid;
  background-color: #FFFFFF;
  border-color: #FFFFFF #5B9BD8 #FFFFFF #5B9BD8;
}

.fourth-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);

}

.fourth-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  font-size: 100%;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);
}

.fifth-line {
  height: 30%;
  width: 100%;
  border-style: solid;
  background-color: #FFFFFF;
  border-color: #FFFFFF #5B9BD8 #FFFFFF #5B9BD8;
}

.fifth-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);

}

.fifth-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  font-size: 100%;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);
}

.sixth-line {
  height: 30%;
  width: 100%;
  border-style: solid;
  background-color: #FFFFFF;
  border-color: #FFFFFF #5B9BD8 #FFFFFF #5B9BD8;
}

.sixth-line-left {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);

}

.sixth-line-right {
  background-color: #FFFFFF;
  width: 50%;
  float: left;
  font-weight: bold;
  font-size: 100%;
  line-height: 100%;
  padding: 0;
  text-align: center;
  position: relative;
  top: 0%;
  /*偏移*/
  transform: translateY(80%);
}

.el-table tr {
  border-color: #5B9BD8;
}
</style>
  