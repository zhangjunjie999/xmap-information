    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['commercial:approval:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['commercial:approval:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['commercial:approval:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['commercial:approval:export']"
        >导出</el-button>
      </el-col>
...
<script>
import { listApproval, getApproval, delApproval, addApproval, updateApproval } from "@/api/commercial/approval";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getApproval(merchantId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除入驻审批 编号为"' + merchantIds + '"的数据项？').then(function() {
        return delApproval(merchantIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('commercial/approval/export', {
        ...this.queryParams
